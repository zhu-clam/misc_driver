...
{
        fd_dma = open(DRIVER_NAME, O_RDWR);
        if(fd_dma < 0){
...
int gv_dma_start(struct dma_chncfg *chncfg)
{
    int ret;
    
    if( fd_dma < 0 || chncfg == NULL)
        return -1;
    ...
    ret = ioctl(fd_dma, AXIDMA_IOCCFGANDSTART_AHB, chncfg);
    if(ret){
...
int gv_hp_dma_start(struct dma_chncfg_sg *chncfg_sg)
{
    int ret;

    
    if( fd_dma < 0 || chncfg_sg == NULL)
        return -1;
    ...
    ret = ioctl(fd_dma, AXIDMA_IOCCFGANDSTART_AXI, chncfg_sg);
    if(ret){
...
int gv_hp_multi_dma_start(GV_DMA_CHN_CFG_MULTI_SG_S *chncfg_multi_sg)
{
    int ret;
    int i;
    if( fd_dma < 0 || chncfg_multi_sg == NULL || chncfg_multi_sg->cnt == 0 || chncfg_multi_sg->cnt > DMA_MAX_SG)
        return -1;
    for(i=0; i<chncfg_multi_sg->cnt; i++){
        if(gv_hp_dma_start(&chncfg_multi_sg->chncfg_sg[i]) < 0 ){
            return -1;
        }
    }
...
void  dma_sg( ){
    ...
    ret = gv_hp_dma_start(fd, &chncfg_sg);
    if(ret){
        printf("ioctl: config and start dma failed\n");
    }
...
void  dma_multi_sg( ){
    ...
    ret = gv_hp_multi_dma_start(fd , &chncfg_multi_sg);
    if(ret){
        printf("ioctl: config and start dma failed\n");
    }
...
int main(void)
{
    ...
    /* open dev */
   // if(gv_dma_device_init() < 0){
    //    printf("open failed\n");
    //    return -1;
   // }


	fd = gv_dma_device_init();
